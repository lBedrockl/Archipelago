            ...
            if(!split[0].includes('Unique location') && !split[0].startsWith("By ") && !split[0].startsWith("Shop ")){
                ...
                if(split[split.length -1].includes("x") && !isNaN(split[split.length -1].at(split[split.length -1].lastIndexOf("x") - 1))){
                    ...
                    if(split[split.length -1].at(split[split.length -1].lastIndexOf("x") - 2) != " "){
                        mult = ' x' + split[split.length -1].slice(split[split.length -1].lastIndexOf("x") -2, split[split.length -1].lastIndexOf("x"))
                    }else{mult = ' x' + split[split.length -1].slice(split[split.length -1].lastIndexOf("x") -1, split[split.length -1].lastIndexOf("x"))}
                    itemInfo[0] += mult
        ...
        if(area != 'unknown'){
            ...
            itemList[area].forEach(item => {
                ...
                switch(true){ // second acro
                    ...
                    case area.includes("isolated_tower_inner"): output += 'IDV'; break
                    case area.includes("_tower"): output += '/DV'; break //fix this if not work
